...
package com.zui.davdroid.common;
...
public class LoggingReader extends FilterReader {
    ...
    // adb shell setprop log.tag.ContactsDAV D
    private static final boolean VERBOSE_LOGGING = Log.isLoggable(TAG, Log.DEBUG);
    ...
    public static Reader getInstance(Reader reader) {
        if (DEBUGGING || VERBOSE_LOGGING) {
            return new LoggingReader(reader);
        } else {
            return reader;
        }
    }

    private LoggingReader(Reader in) {
        this(in, "RAW", false);
    }

    private LoggingReader(Reader in, String tag, boolean dumpEmptyLines) {
        super(in);
        mTag = tag + " ";
    ...
    public static void dump(String tag, String... msgs) {
        if (DEBUGGING || VERBOSE_LOGGING) {
            Log.d(TAG, tag + " dump star");
            for (String msg : msgs) {
                Log.d(TAG, msg);
            }
            Log.d(TAG, tag + " dump end");
        }
    }
}
