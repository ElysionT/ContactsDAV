	...
	private static class ContactsSyncAdapter extends SyncAdapter {
        ...
        public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
            ...
            try {
                ...
                if (service != null) {
                    if (syncCollection)
                        RefreshCollections.refreshCollections(getContext(), account, ServiceDB.Services.SERVICE_CARDDAV, service);
        ...
        private Long getService(@NonNull SQLiteDatabase db, @NonNull Account account) {
            @Cleanup Cursor c = db.query(ServiceDB.Services._TABLE, new String[] { ServiceDB.Services.ID },
                    ServiceDB.Services.ACCOUNT_NAME + "=? AND " + ServiceDB.Services.ACCOUNT_TYPE + "=? AND " + ServiceDB.Services.SERVICE + "=?", new String[] { account.name, account.type, ServiceDB.Services.SERVICE_CARDDAV }, null, null, null);
            if (c.moveToNext())
                return c.getLong(0);
            else
                return null;
