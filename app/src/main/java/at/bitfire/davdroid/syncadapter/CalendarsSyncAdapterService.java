...
public class CalendarsSyncAdapterService extends SyncAdapterService {
	...
	private static class SyncAdapter extends SyncAdapterService.SyncAdapter {
        ...
        public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
            ...
            boolean syncCollection = extras.getBoolean(RefreshCollections.KEY_SYNC_COLLECTION, true);
            try {
                AccountSettings settings = new AccountSettings(getContext(), account);
                if (!extras.containsKey(ContentResolver.SYNC_EXTRAS_MANUAL) && !checkSyncConditions(settings))
                    return;

                updateLocalCalendars(provider, account, settings, syncCollection);
        ...
        private void updateLocalCalendars(ContentProviderClient provider, Account account, AccountSettings settings, boolean syncCollection) throws CalendarStorageException {
            ...
            try {
                ...
                if (syncCollection)
                    RefreshCollections.refreshCollections(getContext(), account, Services.SERVICE_CALDAV, service);
        ...
        Long getService(@NonNull SQLiteDatabase db, @NonNull Account account) {
            @Cleanup Cursor c = db.query(Services._TABLE, new String[] { Services.ID },
                    Services.ACCOUNT_NAME + "=? AND " + Services.ACCOUNT_TYPE + "=? AND " + Services.SERVICE + "=?", new String[] { account.name, account.type, Services.SERVICE_CALDAV }, null, null, null);
            if (c.moveToNext())
                return c.getLong(0);
            else
                return null;
