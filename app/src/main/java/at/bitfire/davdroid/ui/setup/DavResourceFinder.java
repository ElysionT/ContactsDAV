...
public class DavResourceFinder {
    ...
    public DavResourceFinder(@NonNull Context context, @NonNull LoginCredentials credentials) {
        ...
        log = Logger.getLogger("DavResourceFinder");
        log.setLevel(LoggingReader.DEBUGGING ? Level.FINEST : Level.OFF);
    ...
    public Configuration findInitialConfiguration() {
        ...
        return new Configuration(
                credentials.userName, credentials.password, credentials.accountType,
                cardDavConfig, calDavConfig,
                logBuffer.toString(), credentials
        );
    }
...
//        Record[] records = new Lookup(query, Type.SRV).run();
        Lookup srvLookup = new Lookup(query, Type.SRV);
